...
This project is a complete mini data pipeline built for catalog-style pricing and availability analysis. It scrapes live product data from [books.toscrape.com](https://books.toscrape.com/), cleans the scraped values, converts prices from GBP to INR using a fixed project rate, stores the data in a normalized SQLite database, and then analyzes it using both SQL and pandas.
...
1. Reads a JOIN query result using `pd.read_sql(...)`.
2. Loads `books` and `categories` separately.
3. Recreates the JOIN using `pd.merge(...)`.
